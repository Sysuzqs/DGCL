...
class Model_gnn_fp(nn.Module):
    def __init__(self, n_output=1, output_dim=512, dropout=0.2, encoder1=None,encoder2=None):
        ...
        self.weight_fc = nn.Linear(1024,3)
    ...
    def forward(self, data):
        ...
        fp = fp.reshape(len(fp)//1489,1489)
        ecfp = ecfp.reshape(len(ecfp)//1024,1024)
        ...
        weights = self.weight_fc(ecfp)
        weights = F.softmax(weights, dim=1)
        a, b, c = weights[:, 0], weights[:, 1], weights[:, 2]
        
        xf1 = a.unsqueeze(1) * xf1
        xf2 = b.unsqueeze(1) * xf2
        x_fp = c.unsqueeze(1) * x_fp
 
        xf = torch.cat((xf1,xf2,x_fp),dim=1)
